...
class RoleResource extends Resource implements HasShieldPermissions
{
                ...
                Forms\Components\Tabs::make('Permissions')
                    ->tabs([
                        ...
                        Forms\Components\Tabs\Tab::make(__('filament-shield::filament-shield.pages'))
                            ->visible(fn (): bool => (bool) Utils::isPageEntityEnabled() &&
                                (count(FilamentShield::getPages()) > 0 ? true : false))
                            ->live()
                            ->schema([
                                Forms\Components\Grid::make([
                                    'sm' => 3,
                        ...
                        Forms\Components\Tabs\Tab::make(__('filament-shield::filament-shield.widgets'))
                            ->visible(fn (): bool => (bool) Utils::isWidgetEntityEnabled() &&
                                (count(FilamentShield::getWidgets()) > 0 ? true : false))
                            ->live()
                            ->schema([
                                Forms\Components\Grid::make([
                                    'sm' => 3,
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(fn (Builder $query) => $query->whereNotIn('name',
                ['filament_user', 'candidate', 'company']
            ));
    }
    ...
    *----------------------------------*/
                ...
                ->schema([
                    Forms\Components\Toggle::make($entity['resource'])
                        ...
                        ->afterStateUpdated(function (Forms\Set $set, Forms\Get $get, $state) use ($entity) {
                            collect(Utils::getResourcePermissionPrefixes($entity['fqcn']))
            ...
            ->reduce(function ($permissions /** @phpstan ignore-line */, $permission) use ($entity) {
                $permissions[] = Forms\Components\Checkbox::make($permission.'_'.$entity['resource'])
                    ...
                    ->afterStateHydrated(function (Forms\Set $set, Forms\Get $get, $record) use ($entity, $permission) {
                        ...
                        $set($permission.'_'.$entity['resource'],
                            $record->checkPermissionTo($permission.'_'.$entity['resource']));
    ...
    protected static function refreshSelectAllStateViaEntities(Forms\Set $set, Forms\Get $get): void
    {
        $entitiesStates = collect(FilamentShield::getResources())
            ...
            ->when(Utils::isCustomPermissionEntityEnabled(),
    ...
    protected static function refreshResourceEntityStateAfterHydrated(
        Model $record, Forms\Set $set, array $entity
    ): void {
            ...
            $customEntities[] = Forms\Components\Grid::make()
                ->schema([
                    Forms\Components\Checkbox::make($customPermission)
                        ...
                        ->inline()
                        ->afterStateHydrated(
                            function (Forms\Set $set, Forms\Get $get, $record) use ($customPermission) {
                                if (is_null($record)) {
                                    return;
                                }
